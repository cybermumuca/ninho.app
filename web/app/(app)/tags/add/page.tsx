...
export default function AddTagPage() {
  ...
  const form = useForm<AddTagSchema>({
  ...
  function onSubmit(data: AddTagSchema) {
    console.log("Dados da tag:", data);
    router.push("/tags");
  }
    ...
    <div className="flex flex-col min-h-dvh">
      <header className="border-b sticky top-0 bg-background z-10">
        <div className="flex justify-start items-center gap-3 p-4">
          ...
          <h1 className="text-xl font-bold">Criar Tag</h1>
        </div>
      </header>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="max-w-lg mx-auto space-y-6">
            <Card className="p-4">
              ...
              <div className="flex items-center gap-3">
                <Tag
                  name={watchTagName || "nova-tag"}
                  color={selectedColor?.value || "blue"}
                />
              </div>
            </Card>
